# AGENTS.md — Rules for Working on Moto Voice

Guidance for Codex sessions on this repo. Read every session.
...
rounds in a row** before merge. Rider validates — Codex cannot.

If Codex cannot run acceptance (no device access from the sandbox), state so
